...
class ModelSwitchOrchestrator:
    ...
    async def _phase1_update_config_and_restart(self, session: SwitchSession):
        ...
        from core.vllm_manager import _update_vllm_script, refresh_vllm_port_cache, _build_vllm_health_url, get_configured_vllm_port
        ...
        target_port = get_configured_vllm_port()
        ...
        await self._log(session, phase, f"vLLM 服务重启指令已发送，目标端口: {target_port}")
        ...
        while time.time() - start_time < self.PHASE3_START_TIMEOUT:
            ...
            health_url = _build_vllm_health_url(target_port)

            is_port_open = self._is_port_alive(target_port)
            if is_port_open:
                port_seen_alive = True
            elif elapsed >= 30 and not port_seen_alive:
                phase.status = PhaseStatus.FAILED
                phase.error = f"服务重启后 30s 内端口 {target_port} 未打开，服务可能未成功启动"
                phase.finished_at = datetime.now().isoformat()
    ...
    async def _phase2_smoke_test(self, session: SwitchSession):
        ...
        from core.vllm_manager import get_configured_vllm_port, _build_vllm_base_url
        port = get_configured_vllm_port()
    ...
    async def _phase3_start_and_check(self, session: SwitchSession):
        ...
        from core.vllm_manager import _update_vllm_script, refresh_vllm_port_cache, start_vllm_service, _build_vllm_health_url, get_configured_vllm_port
        script_ok = _update_vllm_script(session.target_model_path, session.target_model)
        if not script_ok:
            error_msg = "更新模型路径配置失败"
            phase.status = PhaseStatus.FAILED
        ...
        await self._log(session, phase, f"vLLM 启动指令已发送，目标端口: {target_port}")
        ...
        while time.time() - start_time < self.PHASE3_START_TIMEOUT:
            ...
            try:
                ...
                if is_port_open:
                    ...
                    phase.status = PhaseStatus.FAILED
                    phase.error = f"服务启动后 30s 内端口 {target_port} 未打开"
                    phase.finished_at = datetime.now().isoformat()
                    await self._rollback(session, phase.error)
                    raise _SwitchAborted(phase.error)

                async with httpx.AsyncClient(timeout=3) as client:
                    resp = await client.get(_build_vllm_health_url(target_port))
                    if resp.status_code == 200:
                        phase.status = PhaseStatus.SUCCESS
                        phase.progress = 100
                        phase.finished_at = datetime.now().isoformat()
