...
const getFeatureStatus = (supported: boolean) => {
  return supported ? { icon: CheckCircle, color: 'text-green-500', bgColor: 'bg-green-500/20', text: '支持' } : { icon: XCircle, color: 'text-red-500', bgColor: 'bg-red-500/20', text: '不支持' }
}
  ...
  <div class="bg-slate-800 rounded-xl p-6 h-full flex flex-col">
    ...
    <div class="flex gap-3 mb-4">
      <select
        ...
        :disabled="isTesting"
        class="flex-1 bg-slate-700 text-white border border-slate-600 rounded-lg px-4 py-3 focus:outline-none focus:border-orange-500"
      >
      ...
      <button
        ...
        class="px-6 py-3 bg-orange-600 hover:bg-orange-700 disabled:bg-orange-600/50 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center gap-2"
      >
    ...
    <div v-if="testResult" class="flex-1 overflow-y-auto space-y-4">
      <div class="flex items-center gap-3 p-4 bg-slate-700/50 rounded-lg">
        <div class="w-10 h-10 rounded-full flex items-center justify-center" :class="getStatusBgColor(testResult.status) + '/20'">
          <component
        ...
        <div v-if="testResult.report" class="px-3 py-1 rounded-full text-sm font-medium" :class="getStatusBgColor(testResult.report.overall_status) + '/20 ' + getStatusColor(testResult.report.overall_status)">
          {{ testResult.report.overall_status }}
        </div>
      </div>

      <div v-if="testResult.report?.warnings && testResult.report.warnings.length > 0" class="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
        ...
        <ul class="text-sm text-yellow-300 space-y-1">
          <li v-for="(warning, index) in testResult.report?.warnings" :key="index">- {{ warning }}</li>
        </ul>
      </div>

      <div v-if="testResult.report?.errors && testResult.report.errors.length > 0" class="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
        <div class="flex items-center gap-2 mb-2">
          <XCircle class="w-5 h-5 text-red-500" />
          <span class="text-red-400 font-medium">错误</span>
        </div>
        <ul class="text-sm text-red-300 space-y-1">
          <li v-for="(error, index) in testResult.report?.errors" :key="index">- {{ error }}</li>
        </ul>
      </div>
      ...
      <div v-if="testResult.report" class="space-y-4">
        <div class="grid grid-cols-3 gap-3">
          <div class="bg-slate-700/50 rounded-lg p-4 text-center">
            <div class="w-10 h-10 mx-auto mb-2 rounded-full flex items-center justify-center" :class="getFeatureStatus(testResult.report.feature_support.chat).bgColor">
              <MessageCircle class="w-5 h-5" :class="getFeatureStatus(testResult.report.feature_support.chat).color" />
            </div>
            <p class="text-white font-medium text-sm mb-1">聊天功能</p>
            <span class="px-2 py-0.5 rounded-full text-xs font-medium" :class="getFeatureStatus(testResult.report.feature_support.chat).bgColor + ' ' + getFeatureStatus(testResult.report.feature_support.chat).color">
              {{ getFeatureStatus(testResult.report.feature_support.chat).text }}
            </span>
          </div>

          <div class="bg-slate-700/50 rounded-lg p-4 text-center">
            <div class="w-10 h-10 mx-auto mb-2 rounded-full flex items-center justify-center" :class="getFeatureStatus(testResult.report.feature_support.tool_calling).bgColor">
              <Sparkles class="w-5 h-5" :class="getFeatureStatus(testResult.report.feature_support.tool_calling).color" />
            </div>
            <p class="text-white font-medium text-sm mb-1">工具调用</p>
            <span class="px-2 py-0.5 rounded-full text-xs font-medium" :class="getFeatureStatus(testResult.report.feature_support.tool_calling).bgColor + ' ' + getFeatureStatus(testResult.report.feature_support.tool_calling).color">
              {{ getFeatureStatus(testResult.report.feature_support.tool_calling).text }}
            </span>
          </div>

          <div class="bg-slate-700/50 rounded-lg p-4 text-center">
            <div class="w-10 h-10 mx-auto mb-2 rounded-full flex items-center justify-center" :class="getFeatureStatus(testResult.report.feature_support.multimodal).bgColor">
              <Eye class="w-5 h-5" :class="getFeatureStatus(testResult.report.feature_support.multimodal).color" />
            </div>
            <p class="text-white font-medium text-sm mb-1">多模态</p>
            <span class="px-2 py-0.5 rounded-full text-xs font-medium" :class="getFeatureStatus(testResult.report.feature_support.multimodal).bgColor + ' ' + getFeatureStatus(testResult.report.feature_support.multimodal).color">
              {{ getFeatureStatus(testResult.report.feature_support.multimodal).text }}
            </span>
          </div>
        </div>

        <div v-if="testResult.report.performance_metrics" class="bg-slate-700/50 rounded-lg p-4">
          <h3 class="text-white font-medium mb-3 flex items-center gap-2">
            <Gauge class="w-4 h-4 text-blue-400" />
            性能指标
          </h3>
          <div class="grid grid-cols-3 gap-4">
            <div class="text-center p-3 bg-slate-600/50 rounded-lg">
              <p class="text-slate-400 text-xs mb-1">延迟</p>
              <p class="text-white text-lg font-bold">{{ testResult.report.performance_metrics.latency_ms }}ms</p>
            </div>
            <div class="text-center p-3 bg-slate-600/50 rounded-lg">
              <p class="text-slate-400 text-xs mb-1">吞吐量</p>
              <p class="text-white text-lg font-bold">{{ testResult.report.performance_metrics.throughput }} req/s</p>
            </div>
            <div class="text-center p-3 bg-slate-600/50 rounded-lg">
              <p class="text-slate-400 text-xs mb-1">Token/s</p>
              <p class="text-white text-lg font-bold">{{ testResult.report.performance_metrics.tokens_per_second || '-' }}</p>
            </div>
          </div>
        </div>

        <div v-if="testResult.report.resource_utilization" class="bg-slate-700/50 rounded-lg p-4">
          <h3 class="text-white font-medium mb-3 flex items-center gap-2">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <div class="flex justify-between mb-1">
                ...
                <span class="text-slate-300 text-sm">{{ testResult.report.resource_utilization.gpu_utilization || 0 }}%</span>
              </div>
              <div class="h-2 bg-slate-600 rounded-full overflow-hidden">
                <div
            ...
            <div>
              <div class="flex justify-between mb-1">
                ...
                <span class="text-slate-300 text-sm">{{ testResult.report.resource_utilization.memory_usage || 0 }}%</span>
              </div>
              <div class="h-2 bg-slate-600 rounded-full overflow-hidden">
                <div
        ...
        <div v-if="testResult.report.test_results" class="bg-slate-700/50 rounded-lg p-4">
          <h3 class="text-white font-medium mb-3 flex items-center gap-2">
